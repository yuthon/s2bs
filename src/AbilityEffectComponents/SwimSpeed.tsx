...
import { SwimSpeedProps, SwimSpeedStat } from '../Type';

const SwimSpeed: FC<SwimSpeedProps> = ({ abilityPoint, weapon, abilityName }): ReactElement => {
  let swimSpeed: number | undefined;
  let weight: string = weapon.weight;
  
  // APはabilityPoint  SSはSwimSpeed
  const light: Array<SwimSpeedStat> = [
  ...
  const middle: Array<SwimSpeedStat> = [
  ...
  const heavy: Array<SwimSpeedStat> = [
  ...
  const irregularValues = function(array: Array<SwimSpeedStat>): number | undefined {
    if (abilityPoint === 40) {
