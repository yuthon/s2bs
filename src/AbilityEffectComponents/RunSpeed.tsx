...
import { RunSpeedProps, RunSpeedStat } from '../Type';

const RunSpeed: FC<RunSpeedProps> = React.memo(({ abilityPoint, weapon }): ReactElement => {
  let runSpeed;
  ...
  const Aerospray = Splash;
  const SplatterShot = Splash;
  const Jr = Splash;
  const Gal96 = [
  ...
  const Dualies = L3;
  const Glooga = Gal52;
  const DualieSquelcher = Squeezer;
  const Blob = Luna;
  const Explo = Blaster;
  const Brella = Clash;
  const Tetras = Splash;
  const Tenta = Luna;
  const Undercover = Splash;
  const Nzap = [
  ...
  const irregularValues = function(array: Array<RunSpeedStat>) {
   if (abilityPoint === 40) {
